...
class SemanticMemory:
    ...
    def add_memory(self, content: str, metadata: Optional[Dict[str, Any]] = None):
        """Vectoriza y guarda un nuevo fragmento de memoria."""
        try:
            # Usar litellm para obtener embeddings (por defecto text-embedding-3-small de OpenAI o similar configurado)
            model = os.getenv("EMBEDDING_MODEL", "text-embedding-3-small")
            api_key = os.getenv("OPENAI_API_KEY", os.getenv("GEMINI_API_KEY")) # Fallback a Gemini si es compatible
            
            # Nota: LiteLLM maneja el ruteo. Si el usuario usa Gemini, puede requerir configuración específica para embeddings.
            response = embedding(
                model=model,
                input=[content],
                api_key=api_key
            ...
            conn.commit()
            print(f"[SemanticMemory] Memoria añadida: {content[:50]}...")
        except Exception as e:
            print(f"[SemanticMemory] Error al añadir memoria: {e}")

    def search_similar(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        """Busca fragmentos similares en la memoria usando similitud de coseno."""
        try:
            model = os.getenv("EMBEDDING_MODEL", "text-embedding-3-small")
